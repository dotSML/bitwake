...
| Queue position           | `POST torrents/increasePrio`, `decreasePrio`, `topPrio`, or `bottomPrio`; form `hashes`                                                                                                                                                                                                                                                 | 200; 409 queueing disabled                                                                         | 2.0.0; single/multiple; target 5.2.3 treats literal `all` as an invalid ID and returns a 200 no-op, so Bitwake never sends it |
...
so Bitwake blocks that unsafe edit and directs the user to the native client. This is the
...
Client data is used for versioned Bitwake interface preferences, native/unlocked Media Placement
